...
function animaBoton(clase) {
  ...
  botonOprimido.classList.add(""); //Definir clase que hará la animacion
  setTimeout(function(){
    botonOprimido.classList.remove("");//Definir la calse que se removera
  }, 100);
}

//Añadir los EventListener y llamar a las otras funciones
for (var i=0; i < document.getElementsByTagName("button").lenght; i++) {
  ...
  arregloBotones[i].addEventListener("click", function() {
    ...
    reproducirSonido(idBtnOprimido);
